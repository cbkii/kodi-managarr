...
class Tag:
    ...
    def getFilenameAndPath(self):
        return "smb://pi/Shows/Special.mkv"
...
class KodiSelectedItemTests(unittest.TestCase):
    ...
    def test_unset_db_id_and_year_fall_back_to_labels(self):
        selected = self.selected_with(
            Tag(db_id=0, year="0"),
            {"ListItem.DBID": "42", "ListItem.Year": "2024"},
        )
    ...
    def test_unset_season_and_episode_fall_back_to_labels(self):
        selected = self.selected_with(
            Tag(season=-1, episode="-1"),
            {"ListItem.Season": "0", "ListItem.Episode": "7"},
        )
        self.assertEqual(selected.season, 0)
    ...
    def test_listitem_getpath_fallback_used_before_labels(self):
        ...
        class PathItem:
            def getVideoInfoTag(self):
                return EmptyPathTag(db_id=1)

            def getPath(self):
                return "smb://pi/Movies/Path Film/file.mkv"

        selected = self.selected_with(
            item=PathItem(),
            labels={"ListItem.FileNameAndPath": "smb://pi/Other/file.mkv"},
        )
