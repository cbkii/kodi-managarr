...
class Addon:
    ...
    def __init__(self, **values): self.values = dict(self.defaults); self.values.update(values)
    def getSetting(self, key): return self.values.get(key, "")


class ConfigTests(unittest.TestCase):
    def test_malformed_protected_paths_raise_configuration_error(self):
        with self.assertRaises(ConfigurationError): Settings(Addon(protected_paths="/media/%zz"))

    def test_valid_protected_paths_are_normalised(self):
        settings = Settings(Addon(protected_paths="/media//Movies; smb://PI/Movies"))
        self.assertEqual(settings.protected_paths, ["/media/Movies", "smb://pi/Movies"])

    def test_mapping_roots_are_automatically_protected(self):
        settings = Settings(Addon(path_mappings="/media/Movies=>smb://pi/Movies"))
